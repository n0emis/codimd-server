...
exports.createPDF = function createPDF (req, res, note) {
  ...
  content = body.replace(/\]\(\//g, '](' + url + '/')

  exec('which pandoc', function(err, stdout) {
    var pandocPath;

    if(err || !stdout) {
      logger.error('Cannot find pandoc - is it installed?')
    ...
    args = [];
    args.push('-f', "markdown", '--template', 'eisvogel', '--listings','-o', path);

    pandoc = spawn(pandocPath, args);

    pandoc.stdout.on('data', function(data) {
        //data will be a binary stream if you don't cast it to a string
        logger.info("PANDOC stdout: " + data);
    });

    pandoc.stderr.on('data', function(data) {
        //data will be a binary stream if you don't cast it to a string
        logger.error("PANDOC stderr: " + data);
    });


    pandoc.on('error', function(err) {
        logger.error('Pandoc error: ' + err)
        return errors.errorInternalError(res)
    });
    ...
    //pipe them the input
    pandoc.stdin.write(content, 'utf8');
    pandoc.stdin.end();
  });
}
